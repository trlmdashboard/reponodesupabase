...
module.exports = async (req, res) => {
  try {
    ...
    // Get user info from session (with joined FPC data)
...
<body>
    ...
    <div class="title-bar">
        <div class="title-content">
            ...
            <div class="info-line">FPC: ${userInfo ? userInfo.fpc_name || 'Not assigned' : 'Loading...'}</div>  
        </div>
...
// Get user info from session WITH FPC NAME
async function getUserFromSession(req) {
  try {
    ...
    // Query with JOIN to get fpc_name from 02_fpc table
    const { data: users, error } = await supabase
      .from('01_users')
      .select(`
        login_id,
    ...
    if (error) {
      console.error('Get user database error:', error);
      return null;
    }
    ...
    // Add fpc_name to the user object for easy access
    if (user['02_fpc']) {
      user.fpc_name = user['02_fpc'].fpc_name;
    } else {
      user.fpc_name = null;
    }
